...
$(function () {
  ...
  var kanban_board_data = [{
    //Ici indique le premier kanban board (la section colonne 1)
    id: "kanban-board-1",
    title: "Non traité",
    item: [{
      id: "11",
      title: "Revue des accès utilisateurs aux applications",
      equipe: "Equipe DSI",
      border: "success",
      dueDate: "Feb 6",
      comment: 1,
  ...
  var KanbanExample = new jKanban({
    ...
    itemAddOptions: {
      ...
      content: '+ Ajouter un nouveau plan d\'action',                                                // text or html content of the board button
    ...
    buttonClick: function (el, boardId) {
      ...
      formItem.innerHTML =
        ...
        "</div>" +
        '<div class="form-group">' +
        '<button type="submit" class="btn btn-primary btn-sm mr-50">Envoyer</button>' +
        '<button type="button" id="CancelBtn" class="btn btn-sm btn-danger">Annuler</button>' +
        "</div>";
  ...
  for (kanban_data in kanban_board_data) {
    ...
    for (kanban_item in kanban_board_data[kanban_data].item) {
      ...
      if (typeof $(board_item_el).attr("data-users") !== "undefined") {
        for (kanban_users in kanban_board_data[kanban_data].item[kanban_item].equipe) {
          board_item_users +=
            '<li class="pull-up my-0">' +
            '<span>' +
            kanban_board_data[kanban_data].item[kanban_item].equipe[kanban_users] +
            '"</span>' +
            "</li>";
        }
      }
  ...
  addBoardDefault.addEventListener("click", function () {
    KanbanExample.addBoards([{
      id: "Plan d\'action-" + i, // generate random id for each new kanban
      title: "Action"
    }]);
  ...
  $(".delete-kanban-item").on("click", function () {
   ...
   // $delete_item = kanban_curr_item_id;
    addEventListener("click", function () {
      KanbanExample.removeElement(kanban_curr_item_id);
    });
  });
