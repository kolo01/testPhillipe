...
class TransactionController extends Controller
{
 ...
 {
    $result = DB::table('applications')->select('country')->first();
    $this->country = $result ? $result->country : null;
 }
  ...
  public function transactions()
  {
    ...
    if ($service_status == 1) {
      ...
      $transactions = DB::table('view_transactions')->where('marchand_id', auth()->user()->marchand_id)->where('country', '=', $this->country)->whereDate('created_at', Carbon::today());
      $all_transactions = DB::table('view_transactions')->where('country', '=', $this->country)->whereDate('created_at', Carbon::today());
  ...
  public function SearchTransactions(Request $request)
  {
    ...
    if ($service_status == 1) {
      ...
      $transactions = DB::table('view_transactions')->where('marchand_id', auth()->user()->marchand_id)->where('country', '=', $this->country);
      $all_transactions = DB::table('view_transactions')->where('country', '=', $this->country);
      $infotransaction = DB::table('info_transactions');
    }
  ...
  public function detailtransaction($id)
  {
    ...
    if ($service_status == 1) {
      ...
      $transactions = DB::table('view_transactions');
    }
  ...
  public function statistique()
  {
    ...
    if (auth()->user()->role == 'superAdmin') {
      $trans_pending = DB::table("view_transactions")->where('statut', "INITIATED")->where('country', '=', $this->country)->where("created_at", Carbon::today())->get()->count();
      $sum_pending = DB::table("view_transactions")->where('statut', 'INITIATED')->where("created_at", Carbon::today())->where('country', '=', $this->country)->sum('transacmontant');
      $nb_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->get()->count();
      $sum_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('type', 'depot')->sum('transacmontant');
      $sum_paye = $this->calculFrais($sum_t, $marchand->tranche_transac, "depot");
      $sum_r = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('type', 'retrait')->sum('transacmontant');
      ...
      switch ($this->country) {
        case 'ci':
          ...
          break;
        case 'sn':
          $feesAmount = DB::table('view_montantfrais_total_jour_sn')->first()->montant_total_frais;
          break;
        case 'bf':
          $feesAmount = DB::table('view_montantfrais_total_jour_bf')->first()->montant_total_frais;
          break;        
        default:
          $feesAmount = DB::table('view_montantfrais_total_jour')->first()->montant_total_frais;
          break;
      ...
      $nb_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', auth()->user()->marchand_id)->get()->count();
      $sum_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', auth()->user()->marchand_id)->where('type', 'depot')->sum('transacmontant');
      $sum_paye = $this->calculFrais($sum_t, $marchand->tranche_transac, "depot");
      $sum_r = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', auth()->user()->marchand_id)->where('type', 'retrait')->sum('transacmontant');
      $sum_retire = $this->calculFrais($sum_r, $marchand->tranche_retrait, "retrait");
      $solde = $sum_paye - $sum_retire;
      $f_p = $this->getMtFees($sum_t, $marchand->tranche_transac);
      $f_r = $this->getMtFees($sum_r, $marchand->tranche_retrait);
      //$feesAmount = $f_p + $f_r;
      $sum_pending = DB::table('view_transactions')->where('marchand_id', auth()->user()->marchand_id)->where('statut', 'INITIATED')->where("created_at", Carbon::today())->where('country', '=', $this->country)->sum('transacmontant');
      $trans_pending = DB::table('view_transactions')->where('statut', "INITIATED")->where('marchand_id', auth()->user()->marchand_id)->where("created_at", Carbon::today())->where('country', '=', $this->country)->get()->count();
      $feesAmount = DB::table('view_transactions')->where('statut', 'SUCCESS')->where('marchand_id', auth()->user()->marchand_id)->where('country', '=', $this->country)->sum('montantfrais');
    }
  ...
  public function statistiqueSearch(Request $request)
  {
    ...
    if ((request()->input('periode_debut') == null && request()->input('periode_fin') == null) || (request()->input('periode_debut') > request()->input('periode_fin'))) {
      if (request()->input('marchand_selected') == null) {
        ...
        $nb_t =  DB::table('view_transactions')->where('country', '=', $this->country)->where('marchand_id', $marchandToFind)->where('statut', 'SUCCESS')->get()->count();
        $sum_t =  DB::table('view_transactions')->where('country', '=', $this->country)>where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->where('type', 'depot')->sum('transacmontant');
        $sum_paye = $this->calculFrais($sum_t, $marchandFounded->tranche_transac, "depot");
        $sum_r =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->where('type', 'retrait')->sum('transacmontant');
        $sum_retire = $this->calculFrais($sum_r, $marchandFounded->tranche_retrait, "retrait");
        $solde = $sum_paye - $sum_retire;
        $solde = "";

        $feesAmount = DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->sum('montantfrais');
        $periodeDebut = "";
        $periodeFin = "";
    ...
    if (request()->input('marchand_selected') == null) {
      if (auth()->user()->role == 'superAdmin') {
        ...
        $sum_pending = DB::table('view_transactions')->where('country', '=', $this->country)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $trans_pending = DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', "INITIATED")->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $nb_t =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_t =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_r =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $feesAmount = DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_pending = DB::table('view_transactions')->where('marchand_id', auth()->user()->marchand_id)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $trans_pending = DB::table('view_transactions')->where('statut', "INITIATED")->where('marchand_id', auth()->user()->marchand_id)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_paye = $this->calculFrais($sum_t, $marchand->tranche_transac, "depot");
        $sum_r =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', auth()->user()->marchand_id)->where('type', 'retrait')->sum('transacmontant');
        $sum_retire = $this->calculFrais($sum_r, $marchand->tranche_retrait, "retrait");
        $solde = "";

        $feesAmount = DB::table('view_transactions')->where('country', '=', $this->country)->where('marchand_id', auth()->user()->marchand_id)->where('statut', 'SUCCESS')
          ->when($periodeDebut, function ($query) use ($periodeDebut) {
            return $query->whereDate('created_at', '>=', $periodeDebut);
          })
          ->when($periodeFin, function ($query) use ($periodeFin) {
      ...
      if (auth()->user()->role == 'superAdmin') {
        ...
        $nb_t =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_t =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $sum_r =  DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
        ...
        $feesAmount = DB::table('view_transactions')->where('country', '=', $this->country)->where('statut', 'SUCCESS')->where('marchand_id', $marchandToFind)->when($periodeDebut, function ($query) use ($periodeDebut) {
          return $query->whereDate('created_at', '>=', $periodeDebut);
        })
          ->when($periodeFin, function ($query) use ($periodeFin) {
            return $query->whereDate('created_at', '<=', $periodeFin);
  ...
  public function statistiqueForCommercial()
  {
    ...
    foreach ($nb_total as $key => $value) {
      ...
      $nb_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', $value->id)->get()->count() + $nb_t;
      $sum_t = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', $value->id)->where('type', 'depot')->sum('transacmontant') + $sum_t;
      $sum_paye = $this->calculFrais($sum_t, $value->tranche_transac, "depot") + $sum_paye;
      $sum_r = DB::table('succeed_daily_transactions')->where('country', '=', $this->country)->where('marchand_id', $value->id)->where('type', 'retrait')->sum('transacmontant') + $sum_r;
